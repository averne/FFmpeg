...
/**
 * Angular modes aren't layed out in the same order in hardware and in FFmpeg tables:
 *  - Hardware expects: dc v h d45 d135 d117 d153 d207 d63 tm
 *  - FFmpeg provides:  v h dc d45 d135 d117 d153 d63 d207 tm
 */
static const uint8_t amconv[] = { 2, 0, 1, 3, 4, 5, 6, 8, 7, 9 };

static int envideo_vp9_decode_uninit(AVCodecContext *avctx) {
...
fail:
    envideo_vp9_decode_uninit(avctx);
    return err;
}

static void envideo_vp9_init_probs(nvdec_vp9EntropyProbs_t *probs) {
    int i, j;

    for (i = 0; i < FF_ARRAY_ELEMS(probs->kf_bmode_prob); ++i) {
        for (j = 0; j < FF_ARRAY_ELEMS(probs->kf_bmode_prob[0]); ++j) {
            memcpy(probs->kf_bmode_prob[i][j], ff_vp9_default_kf_ymode_probs[amconv[i]][amconv[j]], 8);
            probs->kf_bmode_probB[i][j][0]   = ff_vp9_default_kf_ymode_probs[amconv[i]][amconv[j]][8];
        }
        memcpy(probs->kf_uv_mode_prob[i], ff_vp9_default_kf_uvmode_probs[amconv[i]], 8);
        probs->kf_uv_mode_probB[i][0]   = ff_vp9_default_kf_uvmode_probs[amconv[i]][8];
    }
}

static void envideo_vp9_update_probs(nvdec_vp9EntropyProbs_t *probs,
                                     VP9Context *s, bool init)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(probs->a.uv_mode_prob); ++i) {
        memcpy(probs->a.uv_mode_prob[i], p->uv_mode[amconv[i]], 8);
        probs->a.uv_mode_probB[i][0]   = p->uv_mode[amconv[i]][8];
    }
...
static void envideo_vp9_set_tile_sizes(uint16_t *sizes, VP9Context *s) {
    int i, j;
...
static void envideo_vp9_update_counts(nvdec_vp9EntropyCounts_t *cts,
                                      VP9TileData *td)
{
    int i, j, k, l;
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(td->counts.y_mode); ++i) {
        for (j = 0; j < FF_ARRAY_ELEMS(td->counts.y_mode[0]); ++j) {
            td->counts.y_mode[i][amconv[j]] = cts->sb_ymode_counts[i][j];
        }
    }

    for (i = 0; i < FF_ARRAY_ELEMS(td->counts.uv_mode); ++i) {
        for (j = 0; j < FF_ARRAY_ELEMS(td->counts.uv_mode[0]); ++j) {
            td->counts.uv_mode[amconv[i]][amconv[j]] = cts->uv_mode_counts[i][j];
        }
    }
                                      ...
                                      EnvideoVP9DecodeContext *ctx, AVFrame *cur_frame)
{
    ...
    FF_ENVIDEO_PUSH_VALUE(cmdbuf, NVC9B0_SET_APPLICATION_ID,
                          DRF_DEF(C9B0, _SET_APPLICATION_ID, _ID, _VP9));
    FF_ENVIDEO_PUSH_VALUE(cmdbuf, NVC9B0_SET_CONTROL_PARAMS,
                          DRF_DEF(C9B0, _SET_CONTROL_PARAMS, _CODEC_TYPE,     _VP9) |
...
static int envideo_vp9_start_frame(AVCodecContext *avctx, const uint8_t *buf, uint32_t buf_size) {
    ...
    av_log(avctx, AV_LOG_DEBUG, "Starting vp9-envideo frame with pixel format %s\n",
    ...
    mem = envideo_map_get_cpu_addr(input_map), common_mem = envideo_map_get_cpu_addr(ctx->common_map);
    ...
    envideo_vp9_update_probs((nvdec_vp9EntropyProbs_t *)(mem + ctx->prob_tab_off), s, ctx->core.new_input_buffer);
    ...
    ctx->refs[2] = ff_envideo_safe_get_ref(h->refs[h->h.refidx[2]].f, h->frames[CUR_FRAME].tf.f);

    return 0;
}

static int envideo_vp9_end_frame(AVCodecContext *avctx) {
    VP9Context                *s = avctx->priv_data;
    ...
    err = envideo_vp9_prepare_cmdbuf(ctx->core.cmdbuf, h, ctx, frame);
    if (err < 0)
        return err;
     ...
     * Since the process depends on entropy counts calculated by the hardware,
