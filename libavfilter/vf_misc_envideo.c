...
typedef struct EnvideoSpatialFilterContext {
    FFEnvideoVppContext core;

    bool has_filter_init;
    EnvideoMap *filter_map;

    int filter_dimensions;
    float sharpness, denoise;
} EnvideoSpatialFilterContext;
...
static void envideo_spatialfilter_uninit(AVFilterContext *avctx) {
    EnvideoSpatialFilterContext *ctx = avctx->priv;

    if (ctx->has_filter_init)
        envideo_map_destroy(ctx->filter_map);

    ff_envideo_vpp_ctx_uninit(avctx);
}

static int envideo_spatialfilter_init_filter_map(EnvideoSpatialFilterContext *ctx) {
    AVHWDeviceContext   *hw_device_ctx = (AVHWDeviceContext *)ctx->core.hw_device_ref->data;
    ...
    err = envideo_map_pin(ctx->filter_map, ctx->core.channel);
    if (err < 0)
        goto fail;

    memcpy(envideo_map_get_cpu_addr(ctx->filter_map), g_VicFilterData, sizeof(g_VicFilterData));
...
static int envideo_spatialfilter_prepare_config(EnvideoSpatialFilterContext *ctx, VicConfigStruct *config) {
...
static int envideo_spatialfilter_prepare_cmdbuf(EnvideoSpatialFilterContext *ctx,  AVEnvideoJob *job,
...
static int envideo_spatialfilter_filter_frame(AVFilterLink *link, AVFrame *in) {
    ...
    if (!ctx->has_filter_init) {
        err = envideo_spatialfilter_init_filter_map(ctx);
        if (err < 0)
            return err;
    }
    ...
    config = (VicConfigStruct *)((uint8_t *)envideo_map_get_cpu_addr(map) + ctx->core.vic_setup_off);
    ...
    err = envideo_spatialfilter_prepare_config(ctx, config);
    if (err < 0)
        goto fail;
    ...
    err = envideo_spatialfilter_prepare_cmdbuf(ctx, job, in, out);
    if (err < 0)
        goto fail;
...
#define SOFFSET(x) offsetof(EnvideoSpatialFilterContext, x)
#define FLAGS (AV_OPT_FLAG_VIDEO_PARAM | AV_OPT_FLAG_RUNTIME_PARAM | AV_OPT_FLAG_FILTERING_PARAM)

static const AVOption envideo_sharpness_options[] = {
...
static const AVOption envideo_denoise_options[] = {
...
AVFILTER_DEFINE_CLASS(envideo_sharpness);
AVFILTER_DEFINE_CLASS(envideo_denoise);

static const AVFilterPad envideo_spatialfilter_inputs[] = {
    {
        .name         = "default",
        .type         = AVMEDIA_TYPE_VIDEO,
        .filter_frame = &envideo_spatialfilter_filter_frame,
        .config_props = &ff_envideo_vpp_config_input,
    },
};

static const AVFilterPad envideo_spatialfilter_outputs[] = {
    {
        .name         = "default",
        .type         = AVMEDIA_TYPE_VIDEO,
